...
test: beautiful
	conda run -n base python make/env.py test_tofunc --python=3.11 --recreate >/dev/null;
	conda run -n test_tofunc pip install -e . >/dev/null;
	conda run -n test_tofunc python run_tests.py;
	conda run -n test_tofunc pip install mypy >/dev/null;
	conda run -n test_tofunc python -m mypy --exclude build --exclude dist --strict .;
	conda run -n test_tofunc python -m mypy --strict -p tofunc;
